---
id: codex-readiness-profile-20260713
title: Codex Local Runtime Assets 项目画像候选
...
summary_zh: 记录 Codex Local Runtime Assets 的 registry 身份、仓库边界、已有 Hub 证据和权威边界；只作 reviewing 工作台，不声明源码或发布事实。
promotion_decision: none; structural readiness asset only, no active promotion or owner decision
path: domains/codex/current/project-profile.md
project_id: codex
...
|---|---|
| project_id | `codex` |
| 类型 | `runtime-repository` |
| repo boundary | `runtime-assets` |
| 所属组 | `codex` |
...
|---|---|---|---|
| `codex` | `jiying2007/codex` | `~/codex` | `first-party` |
...
- 当前没有可复用的已登记正文；只保留项目 registry 身份，不推断源码事实。
