---
id: codex-readiness-runbook-20260713
title: Codex Local Runtime Assets 维护入口
kind: runbook
domain: codex
scope: team-general
...
summary_zh: 定义 Codex Local Runtime Assets 的 Knowledge Hub 预检、候选写入、验证与授权边界，不生成未经源项目确认的工程命令。
promotion_decision: none; owner boundary attested, no active promotion or evidence-ready claim
path: domains/codex/current/runbooks/maintenance-entry.md
project_id: codex
...
- domains/codex/current/project-profile.md
- domains/codex/decisions/project-boundary-decision-candidate.md
- domains/codex/validation/project-readiness.md
---

# Codex Local Runtime Assets 维护入口
...
```bash
rtk bash ~/knowledge-hub/tools/knowledge-context.sh --cwd "$PWD" --query "codex 当前事实与验证" --task-type validation --json
rtk bash ~/knowledge-hub/tools/knowledge-search.sh "codex" --json --limit 10
```

确认 route 的 `selected_project_id=codex`，并区分 `current`、`recent` 与 archive-only provenance。若本机只有 `workspace://` 而没有 local mapping，先补只读映射或由 owner 提供源码证据，不猜测路径。
