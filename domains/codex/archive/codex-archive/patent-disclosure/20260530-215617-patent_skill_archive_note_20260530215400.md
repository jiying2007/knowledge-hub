# Patent Disclosure Skill And Patent Portfolio Archive
...
- Archive purpose: preserve the reusable patent disclosure workflow, generated artifacts, validation evidence, and quality improvement recommendations for four patent candidates.
...
### 3. Read-only Customer Region And Writable Data Region Safe OTA Migration

- Disclosure Markdown: `一种宠物机器人只读客户区与可写数据区协同的安全OTA迁移方法及系统_20260530204854.md`
- Disclosure Word: `一种宠物机器人只读客户区与可写数据区协同的安全OTA迁移方法及系统_20260530204854.docx`
- Claims Markdown: `权利要求书_一种宠物机器人只读客户区与可写数据区协同的安全OTA迁移方法及系统_20260530204854.md`
- Claims Word: `权利要求书_一种宠物机器人只读客户区与可写数据区协同的安全OTA迁移方法及系统_20260530204854.docx`
- Figure/formula assets: `figures_ota/`

Quality status: strong reliability and release-engineering patent candidate. It is practical and evidence-backed, but prior-art risk is higher because UBI, SquashFS, ubiblock, and block OTA mechanisms are known individually. The inventive angle should stay on the coordinated migration and verification closure for a robot resource layout.

Recommended improvements:

1. Avoid claiming UBI/SquashFS/ubiblock primitives too broadly; claim the coordinated OTA migration policy and validation closure.
2. Add a migration state table: old layout, transitional package, post-migration layout, customer-only update.
3. Add bootargs preservation/injection pseudo-code in the disclosure support section.
4. Add failure evidence and recovery observations as implementation effects, not as broad legal conclusions.

### 4. Edge-side Multimodal Companion Behavior Orchestration
...
Overall quality: good enough for attorney review and first-round patentability discussion.
...
3. The strongest filing order is likely:
   ...
   - Safe OTA migration.
4. The OTA filing has the highest prior-art density and should be drafted with narrower coordination/verification language.
...
1. Four disclosure Word files were generated.
2. Four claims Word files were generated.
3. Mermaid diagrams and formulas were rendered to PNG for the disclosure documents.
4. Historical residual files for the three pet-robot patents were removed.
5. `rtk bash ~/codex/scripts/final-ready.sh` passed during the patent delivery workflow.
...
3. Add implementation evidence tables for measured resource savings, production-line yield/time, OTA failure reduction, and behavior engagement metrics.
