...
The 2026-06-22 update records that `libmsc.so errno=5`, `wait bdma done timeout`, and `CamOsTsemUp` are not one same-reboot direct chain, but are correlated as the same FSP/QSPI/BDMA/MTD read-path failure family.
The 2026-06-23 update records a positive 4-device 12-hour debug auto-reboot aging result with no `libmsc.so warmup read failed` or App signal 7 recurrence on the current CK 2mA / IO 4mA plus BDMA hardening baseline; this is a stabilization candidate, not yet a final root-cause closure.
