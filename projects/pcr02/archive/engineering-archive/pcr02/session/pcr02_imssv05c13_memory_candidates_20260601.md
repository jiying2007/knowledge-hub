...
- Source scope: current session wrap, `/vsdata/leiwenjun/Iford_IMSSV05C13` commits, PCR02 validation archive
- Mode: report-only
...
| Project fact | Target SDK migration repository is `/vsdata/leiwenjun/Iford_IMSSV05C13`; current product source is `/vsdata/leiwenjun/pcr02_ssc305_compile`; older maintained baseline comparison is `/vsdata/leiwenjun/Iford_IMD00V5.1.1_20250529`. | Session wrap repository table. | Medium, path may be local-machine specific | High | Archive-only or machine-local memory after confirmation. |
