---
id: agent-dev-kit-readiness-runbook-20260713
title: Agent Dev Kit 维护入口
kind: runbook
...
summary_zh: 定义 Agent Dev Kit 的 Knowledge Hub 预检、候选写入、验证与授权边界，不生成未经源项目确认的工程命令。
promotion_decision: none; structural readiness asset only, no active promotion or owner decision
path: projects/agent-dev-kit/current/runbooks/maintenance-entry.md
project_id: agent-dev-kit
readiness_slot: runbook
aliases:
- agent-dev-kit runbook
- agent-dev-kit-runbook
related:
- projects/agent-dev-kit/README.md
- projects/agent-dev-kit/current/project-profile.md
...
```bash
rtk bash ~/knowledge-hub/tools/knowledge-context.sh --cwd "$PWD" --query "agent-dev-kit 当前事实与验证" --task-type validation --json
rtk bash ~/knowledge-hub/tools/knowledge-search.sh "agent-dev-kit" --json --limit 10
```

确认 route 的 `selected_project_id=agent-dev-kit`，并区分 `current`、`recent` 与 archive-only provenance。若本机只有 `workspace://` 而没有 local mapping，先补只读映射或由 owner 提供源码证据，不猜测路径。
