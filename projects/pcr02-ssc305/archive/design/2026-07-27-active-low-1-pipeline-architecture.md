---
id: pcr02-active-low-1-pipeline-architecture-20260727
title: PCR02 SSC305 ACTIVE_LOW_1组件化Pipeline与日夜策略
kind: architecture
domain: projects/pcr02-ssc305
path: projects/pcr02-ssc305/archive/design/2026-07-27-active-low-1-pipeline-architecture.md
...
source:
  ...
  source_sha256: ecf3a9c9df59a51077be132268c91236bfa489d78880afe2826e8224395d10b3
...
summary_zh: SSC305在SOC常醒条件下只保留OFF、ACTIVE_LOW_1和NORMAL_30；1/30fps物理切换使用COLD rebuild，1fps禁用LDC/软光敏并强制白天，30fps AUTO无H26x流时强制白天。
...
```text
soft_light = (fps == 30) && charging && (light_mode == AUTO)
```
...
| 30 | 任意 | AUTO | 持续无流 | 按充电状态选择 | `H26xStreamGate` 强制白天 | 按充电状态选择 |
...
- `NORMAL_30 + AUTO` 保留 `H26xStreamGate`：编码流停止后经过既有 6 次检查宽限，强制切回白天；强制切换不再叠加 AUTO 的 3 秒最小驻留。
