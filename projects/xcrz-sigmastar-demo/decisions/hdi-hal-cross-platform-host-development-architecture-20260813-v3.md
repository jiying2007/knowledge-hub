...
decision_id: pcr02-hdi-hal-cross-platform-host-development-architecture-20260813-v3
supersedes_candidate: pcr02-hdi-hal-cross-platform-host-development-architecture-20260813-v2
...
id: pcr02-hdi-hal-cross-platform-host-development-architecture-20260813-v3
title: HDI/HAL/Diag 跨平台硬切换与 Host 自动化架构决策候选 v3
kind: decision
domain: projects/xcrz-sigmastar-demo
path: projects/xcrz-sigmastar-demo/decisions/hdi-hal-cross-platform-host-development-architecture-20260813-v3.md
...
source:
  type: design-review
  from: hard-cut architecture review with repository static evidence
  source_sha256: e458a33edb692f37a874aa23e17324e2e65c8109eb0c4cc1fd7b83680a814875
...
- decision-candidate
validation_refs:
- projects/xcrz-sigmastar-demo/decisions/hdi-hal-cross-platform-host-development-architecture-20260813-v3.md
...
summary_zh: v3 将跨平台重构收敛为唯一目标态：当前 Make 为唯一构建权威，C0-C6 仅作隔离分支 checkpoint，产品分支硬切换；无 compat/alias/shim/fallback/双 catalog/动态命令注册，多
  profile 使用独立制品，永久 Host target 与 blocking 自动门禁负责零残留和回归。
primary_language: zh-CN
source_language: zh-CN
translation_status: not-required
terminology_status: pending-review
aliases:
- HDI/HAL/Diag 跨平台硬切换与 Host 自动化架构决策候选 v3
related:
...
该决策已通过硬切换与零残留独立 review，只代表架构候选，不代表平台实现、实板验证、负责人批准或发布完成。
...
这些事项不阻塞方案成为 reviewing candidate，但会阻塞对应 target 的支持声明；任何未回答项均 fail closed，不产生兼容默认值。
...
- 完成条件：DG-01 至 DG-20、production catalog/symbol audit、Host negative matrix 和 SSC305 HIL 全部通过。
...
- 生成切换 receipt，绑定源码、manifest、生成物、构建、Host、HIL 和发布制品 hash；
- 只在 C6 完整通过后一次合入产品分支。回滚为整体 revert 或上一版发布制品，不在新代码中恢复旧路径。

### Target certification

- T-SSC305：C6 的首个量产证明，contract + 产品回归 + HIL + catalog/symbol/包审计必须通过；
...
- AC-I9：Diag DG-01 至 DG-20 全部有 Host/target 对应证据。
