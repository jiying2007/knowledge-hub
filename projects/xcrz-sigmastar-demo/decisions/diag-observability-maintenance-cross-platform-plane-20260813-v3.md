...
decision_id: pcr02-diag-observability-maintenance-cross-platform-plane-20260813-v3
supersedes_candidate: pcr02-diag-observability-maintenance-cross-platform-plane-20260813-v2
related_review: pcr02-hdi-hal-diag-final-architecture-determination-20260813
created: 2026-08-13
updated: 2026-08-13
related_decision: pcr02-hdi-hal-cross-platform-host-development-architecture-20260813-v4
platforms:
- SSC305
- SSU9383CM
- RDK X5
...
id: pcr02-diag-observability-maintenance-cross-platform-plane-20260813-v3
title: Diag Observability Maintenance 跨平台最终平面规范 v3
kind: decision
domain: projects/xcrz-sigmastar-demo
path: projects/xcrz-sigmastar-demo/decisions/diag-observability-maintenance-cross-platform-plane-20260813-v3.md
...
source:
  ...
  from: final Diag architecture and security determination
  source_sha256: 3d914bc8b28a7d2e061f2405eee694dedc4d4d777297703709d376f6d35aee21
...
- security
validation_refs:
- projects/xcrz-sigmastar-demo/decisions/diag-observability-maintenance-cross-platform-plane-20260813-v3.md
...
summary_zh: 冻结 Diag Common/runtime/adapter/use-case 边界、单 profile 制品信任、生命周期安全、零兼容与 DG-01 至 DG-22 自动门禁。
primary_language: zh-CN
source_language: zh-CN
translation_status: not-required
terminology_status: pending-review
aliases:
- Diag Observability Maintenance 跨平台最终平面规范 v3
related:
...
9. production、service、factory、product_test 是由编译期 catalog、target identity、独立签名策略、设备生命周期及 boot/update policy 共同确定的不同制品；profile 不得由请求、配置、普通 OTA 或运行时切换。
...
- 本规范不定义凭据签发、PKI 或云端账号系统，只定义认证 adapter 必须输出的已验证 principal、不可变 permission set 与 authentication strength contract；runtime 不消费外部 role。
...
### 11.1 Profile 制品信任与设备生命周期

- target identity 至少覆盖 product/board/soc/target/profile、catalog/policy/limits hash、toolchain、版本、lifecycle class、signing policy 和 anti-rollback epoch；缺字段不得打包或启动。
- factory/product_test signing policy 只在 manufacturing 生命周期有效；设备转入 production 后，boot chain、recovery 和 updater 必须不可逆地拒绝其 key、profile 和 target identity。
- production OTA 只能更新同 product/board 的 production profile，写入前和启动时双重校验 identity、签名、epoch；不得把“升级”作为 profile 切换通道。
- service 制品只能走受信 recovery/临时启动通道，不写入 production 正常槽，不进入普通 OTA；同时要求物理或本地授权、设备 nonce 绑定、短期单次 ticket、到期失效、审计和超时/重启后自动返回 production。
- service/factory profile 仍须逐命令执行 permission、authentication strength、device state、confirmation 和 audit；profile 身份本身不授予权限。
- Host 完整覆盖 metadata 与拒绝矩阵；真实 secure boot、update、lifecycle transition、anti-rollback 和恢复路径由每个目标 HIL 证明，缺证据不得发布对应制品。
...
| target/profile binary | product/board/profile/lifecycle/signing/epoch 精确匹配；普通 OTA 不切 profile |
...
除上述持久化 migrator 外，仓内源码 ABI、Common/runtime ABI、wire/schema、命令/CLI、产测/售后工具和 target/profile 制品均无兼容实现；全部消费者由 cutover inventory 同包切换。受控 service 临时启动属于运维授权路径，不是旧版本兼容路径。
...
- 完成条件：DG-01 至 DG-22、Host、SSC305 HIL、profile 制品生命周期拒绝矩阵、生成物和发布制品 receipt 全部通过。回滚仅允许整体制品/提交回退。
...
- Presubmit 阻断 architecture/generated、Host unit/contract/integration、sanitizer、TSan、fuzz smoke 和 coverage；target integration 阻断交叉构建、产物审计、portable contract 与 profile identity 拒绝矩阵；release certification 阻断完整 HIL、性能/质量、secure boot/update/lifecycle/anti-rollback 和完整 fuzz corpus。
- HIL unavailable、runner/设备/网络基础设施错误和未执行均不是通过；固定重试预算耗尽后保持 BLOCKED/FAIL，不允许绿色降级。
- 测试隔离必须有 owner、缺陷、创建日期和最长 14 天 expiry；安全、契约、零残留、生成确定性和 profile 生命周期测试禁止隔离，过期隔离直接阻断。
- coverage 排除仅限生成物和批准的 third_party/vendor；阈值与 changed-line/branch 不回退同时满足，排除列表变化必须 code owner 审查。
...
- DG-21：每个 profile 制品的 target/signing/lifecycle/epoch identity 完整；量产设备拒绝 factory/product_test，普通 OTA 不能改变 profile，service 仅能受控临时启动并自动返回 production。
- DG-22：Presubmit、target integration、release certification 三层门禁生成 identity-bound receipt；基础设施失败、过期证据、skip、unsupported 和过期隔离不能计为通过。
...
| 量产设备装载 factory/product_test 或 OTA 改 profile | blocker | 独立签名策略、设备生命周期、boot/update 双阶段校验、anti-rollback HIL |
| HIL/基础设施失败或隔离测试被当作通过 | blocker | 三层阻断门禁、固定重试、隔离 expiry、identity-bound receipt |
...
| artifact identity/signing/lifecycle manifest | release trust SSOT | profile key policy、设备 lifecycle、boot/update allowlist、anti-rollback epoch |
| target release_policy manifest | certification SSOT | blocking jobs、重试预算、证据最大时效、禁止隔离类别与 receipt schema |
...
- production/service/factory/product_test 的 policy/limits/catalog 均由 target manifest 生成并编译内置，只读且绑定 binary identity；Diag 不能修改自身 policy；
...
本规范是最终设计基线并已通过最终架构、安全与硬切换 review。归档治理状态保持 reviewing，等待 team-core、security、product、platform owner 的正式 attestation；它作为总体 HDI/HAL 跨平台方案的强制组成部分，不为未实现功能、实板 certification 或发布背书。
