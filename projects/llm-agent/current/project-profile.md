---
id: llm-agent-readiness-profile-20260713
title: LLM Agent 项目画像候选
...
summary_zh: 记录 LLM Agent 的 registry 身份、仓库边界、已有 Hub 证据和权威边界；只作 reviewing 工作台，不声明源码或发布事实。
promotion_decision: none; structural readiness asset only, no active promotion or owner decision
path: projects/llm-agent/current/project-profile.md
...
|---|---|---|---|
| `llm-agent` | `jiying2007/llm_agent` | `workspace://llm-agent` | `first-party` |
...
- [LLM Agent readiness validation](../validation/project-readiness.md)：`reviewing` / `validation`
- [LLM Agent 权威与维护边界决策候选](../decisions/project-boundary-decision-candidate.md)：`reviewing` / `decision`
- [LLM Agent 维护入口](runbooks/maintenance-entry.md)：`reviewing` / `runbook`
- [LLM Agent 项目画像候选](project-profile.md)：`reviewing` / `project-current`
