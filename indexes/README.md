...
- `artifacts/manifests/` 不维护完整人工索引；恢复最新 manifest、Markdown/JSONL 配对、行数和证据计数时运行 `rtk bash ~/knowledge-hub/tools/knowledge-index-plan.sh --section manifest --json`，只在确有长期导航价值时把摘要登记到 registry/index。manifest latest 只按文件名 `YYYYMMDD` 排序，row 内日期只作为 `row_date` 辅助字段，不能让旧文件凭 row 日期抢占最新恢复依据。`profile_health` 是 report-only 恢复提示，`summary_source` / `evidence_source` 只说明摘要和证据数从哪个 JSONL 字段派生；旧制品的 `legacy-missing-profile`、引用类的 `reference-only` 和 `missing-boundary` 提示不等同于硬失败，也不要求回填历史正文。
