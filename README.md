...
2. `final_status=needs-owner-review` 时，只在 `automatic_governance.status=complete-except-owner-review` 且 `gap_map[]` 只有 `owner-gates-open` 时停给真实 owner；按 `owner_recovery.owner_dispatch[].suggested_owner_packet` 领取、导出、校验和生成 landing plan。
