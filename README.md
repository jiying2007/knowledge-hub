...
5. 如涉及 owner-gated 内容，先运行 `rtk bash ~/knowledge-hub/tools/knowledge-owner-gates.sh --source-id <source-id>`，确认 owner、必填证据和 active exposure；需要一次性分派全部 open gate 时加 `--summary`，需要 owner 签收上下文时加 `--checklist`，需要人读说明和可复制 JSONL 表单时加 `--forms`，需要只给脚本或人工保存的纯 JSONL 表单时加 `--forms-jsonl`。
   也可以先运行 `rtk bash ~/knowledge-hub/tools/knowledge-status.sh --json`，读取 `owner_gates.summary_commands` 查看全部 open gate，用 `owner_gates.forms_jsonl_commands` 导出全部纯 JSONL 空白表单，用 `owner_gates.validate_forms_commands` 校验 owner 填好的 JSONL，用 `owner_gates.landing_plan_commands` 生成无写入人工 landing plan，再用 `owner_gates.next_open.next_open_command` 聚焦下一条 owner gate；或直接运行 `rtk bash ~/knowledge-hub/tools/knowledge-owner-gates.sh --source-id <source-id> --next-open --checklist --forms`。
