...
2. 正向 fixture：填写 `owner_decision=reference-only`、`target_decision=reference-only`、`reviewed_by`、`reviewed_at=2026-06-19`、`source_status`、`source_sha256`、`source_size`、`current_validity`、`scope_statement`、`applicable_project_version`、非空 `evidence_refs` 和 `status_reason`，写入 `/tmp/kh-owner-form-valid*.jsonl`。
3. 负向 fixture：基于正向 fixture，把 `owner_decision` 改为不在 `allowed_owner_decisions` 中的值，并清空 `source_sha256`，写入 `/tmp/kh-owner-form-invalid*.jsonl`。
...
- 下一次复核内容：若 owner decision landing 规则增加新字段、禁用组合或跨字段语义约束，同步校验规则。
