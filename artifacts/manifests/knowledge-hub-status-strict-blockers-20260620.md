...
| `rtk bash tools/knowledge-regression.sh --json` | 0 | 通过；20 个回归场景全部 pass，`status-next-owner-gate` 覆盖 strict blocker，`owner-form-source-identity-mismatch` 覆盖过期源文件身份拒绝，`final-gate-owner-review-blocker` 覆盖终态 gate 聚合。 | `tools/knowledge-regression.sh` | Regression | `knowledge-hub-status-strict-blockers-20260620` |
