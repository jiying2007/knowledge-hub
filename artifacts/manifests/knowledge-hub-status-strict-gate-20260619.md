...
| KSS-002 | 终态目标要求 owner-gated 残留不得被忽略。 | PCR02 7 个 owner-gated 源文件仍 open 时，不能声明全部迁移落地完成。 | strict 模式要求 `status=ok` 才返回 0，并通过 `strict_blockers` 暴露阻塞原因。 |
...
- JSON 输出包含 `strict` 和 `strict_blockers` 字段，便于自动化判断本次运行模式和终态阻塞原因。
...
| --- | --- | --- | --- | --- | --- |
| `rtk bash tools/knowledge-status.sh --json` | 0 | 默认模式保持日常看板语义；当前 `status=needs-owner-review`、`strict=false`、`open_count=7`，并含 `strict_blockers[0].id=owner-gates-open`。 | `tools/knowledge-status.sh` | Knowledge Hub | status-strict-gate |
| `rtk bash tools/knowledge-status.sh --strict --json` | 1 expected | strict 模式在当前 `needs-owner-review` 下返回 1；JSON 中 `strict=true`、`open_count=7`，`strict_blockers` 给出 owner summary 和 next-open 命令。 | `tools/knowledge-status.sh` | Final-state gate | status-strict-gate |
...
- 下一次复核内容：若终态定义新增更多阻塞类型，应同步 strict 模式退出码规则、`strict_blockers` 枚举和 status dashboard 字段。
