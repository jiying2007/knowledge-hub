...
当前 regression helper manifest 已同步到 16 个回归场景，并列出所有当前测试 ID。

## 问题地图

| ID | 问题 | 风险 | 处理 |
|---|---|---|---|
| RMC-001 | `knowledge-regression.sh` 已扩展到 16 个场景，但 regression helper manifest 曾滞后。 | 长期审计会误判回归覆盖范围，后续维护者不知道实际检查了什么。 | 同步 regression helper manifest 到 16 个场景。 |
...
|---|---:|---|---|---|---|
| `rtk bash tools/knowledge-regression.sh --json` | 0 | 通过；16 个回归场景全部 pass，包含 `owner-checklist-context`、`owner-form-context`、`owner-next-open-focus`、`manual-entry-docs-owner-option` 和 `regression-manifest-coverage` 场景 | `tools/knowledge-regression.sh` | Tool | `knowledge-hub-regression-manifest-coverage-20260619` |
