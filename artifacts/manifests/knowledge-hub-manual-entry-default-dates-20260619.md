...
| ID | 问题 | 风险 | 处理 |
|---|---|---|---|
| MDD-001 | 人工新增向导的 registry 和 source policy 草稿使用 `<YYYY-MM-DD>` 占位符。 | 人工复制时容易漏替换日期，导致 registry date gate 失败或复核周期缺失。 | 自动生成默认 ISO 日期。 |
...
|---|---:|---|---|---|---|
| `rtk bash tools/knowledge-new.sh --kind decision --domain governance --id governance-date-defaults --path governance/date-defaults.md` | 0 | 通过；registry 和 source policy 草稿输出默认 ISO 日期，不再保留日期占位符 | `tools/knowledge-new.sh` | Tool | `knowledge-hub-manual-entry-default-dates-20260619` |
