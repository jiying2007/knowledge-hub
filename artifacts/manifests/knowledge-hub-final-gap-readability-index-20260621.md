...
- 新增 2026-06-21 起的 migration 中文可读性门禁：source policy row 必须带 `notes_zh`。
...
- `tools/knowledge-check.sh`
  ...
  - 对 2026-06-21 及之后新增的 source policy row 执行 `notes_zh` 门禁。
- `tools/knowledge-regression.sh`
  ...
  - 新增 source policy `notes_zh` 负向 fixture。
  - 回归场景从 56 个提升到 60 个。
- `registry/schema.md`
  - 说明 2026-06-21 后 source policy row 的 `notes_zh` 要求。
- `registry/items.jsonl`
  - 为 2026-06-21 已有 source policy row 补中文说明。
...
- 新增或更新日期在 2026-06-21 及之后的 governance audit registry item 必须包含：
  ...
  - `translation_status`
  - `terminology_status`
- 新增或更新日期在 2026-06-21 及之后的 source policy row 必须包含：
  - `notes_zh`
...
| `rtk bash ~/knowledge-hub/tools/knowledge-check.sh --dry-run --json --diagnostics` | 0 | 通过；新增中文可读性门禁、registry、source policy 和 index 登记后全仓无 errors / warnings。 | `tools/knowledge-check.sh` | Tool | `knowledge-hub-final-gap-readability-index-20260621` |
| `rtk bash ~/knowledge-hub/tools/knowledge-regression.sh --json` | 0 | 通过；60 个回归场景全部 pass，包含 typed registry gap、final gap/readability 正向契约、governance audit readability gate 和 source policy notes_zh gate。 | `tools/knowledge-regression.sh` | Tool | `knowledge-hub-final-gap-readability-index-20260621` |
