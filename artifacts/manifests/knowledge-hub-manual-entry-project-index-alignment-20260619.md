...
`knowledge-new.sh` 和 `templates/README.md` 已补充项目域条目的 `indexes/by-project.md` 导航入口要求。人工新增项目知识时，向导不再只提示 registry、owner/date/status 索引和 migration，而会明确提醒维护项目导航索引。
...
| MPI-003 | 仅靠文档提示容易退化。 | 后续修改向导时可能再次漏掉项目索引。 | 在 `knowledge-regression.sh` 增加 `manual-entry-project-index-hint` 场景。 |

## 决策

- `knowledge-new.sh` 仍然只读，人工仍可直接按模板新增内容。
- 项目域条目提示 `indexes/by-project.md`，但不把 `by-project` 改成强制覆盖每个 registry item 的门禁。
- 回归只校验项目域示例输出包含 `indexes/by-project.md` 和目标正文路径。
