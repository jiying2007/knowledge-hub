...
| MCS-004 | 项目域 item 容易只补 registry 和核心 status 索引，漏掉 `indexes/by-project.md`。 | 项目导航入口缺失，人工查找 PCR02 等项目知识时需要回到 registry 搜索。 | `knowledge-new.sh` 输出项目索引提示，回归覆盖 PCR02 项目条目示例。 |
...
- 核心索引草稿提示 `by-owner`、`by-review-date` 和 `by-status` 的人工登记位置；项目域条目额外提示 `by-project` 导航入口。
...
- `rtk bash tools/knowledge-regression.sh --json`：通过，manual project entry 回归确认项目域新增向导输出 `indexes/by-project.md`。
