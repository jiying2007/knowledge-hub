...
| regression-manifest-coverage | 当前回归 helper manifest 覆盖所有回归 ID | manifest 包含 16 个回归场景和所有当前测试 ID |
...
|---|---:|---|---|---|---|
| `rtk bash tools/knowledge-regression.sh --json` | 0 | 通过；16 个回归场景全部 pass，覆盖 status 负向 fixture、owner gate 负向 fixture、owner form 聚焦、owner checklist、owner form 上下文、owner next-open 聚焦、status next owner gate、owner landing plan、manual entry 防漏、owner 文档可发现性和 regression manifest 自检 | `tools/knowledge-regression.sh` | Tool | `knowledge-hub-governance-regression-helper-20260619` |
