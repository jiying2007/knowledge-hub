...
| `rtk bash ~/knowledge-hub/tools/knowledge-regression.sh --json --as-of 2026-06-23` | 0 | 通过；119 个回归场景全部 pass，新增 `owner-handoff-packet-json` 和 `manifest-profile-boundary-advisory`，并保留 `owner-dispatch-source-scope-isolation` 与 `manifest-regression-count-capture-qualifier`，证明 owner 一包化 handoff packet 可离线签收但不生成 decision，manifest boundary profile 缺失只作为 advisory 恢复提示，owner 分派按 source scope 隔离，历史 manifest 固定回归计数带当次捕获口径。 | `tools/knowledge-regression.sh` | Tool | `knowledge-hub-governance-regression-helper-20260619` |
