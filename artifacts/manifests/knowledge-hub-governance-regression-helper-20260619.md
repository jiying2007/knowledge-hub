...
| `rtk bash ~/knowledge-hub/tools/knowledge-regression.sh --json --as-of 2026-06-23` | 0 | 通过；108 个回归场景全部 pass，新增 `final-proof-artifacts-stable-alias`、`owner-validate-forms-partial-coverage-warning` 和 `status-owner-ready-source-no-registry-fallback`，证明 final gate 的 canonical `proof_artifacts` 与 legacy `proof_artifacts_20260622` 内容一致，owner validate/landing 对分批签收给出 partial coverage 提示，并证明 status owner queue 不再从 registry item presence 推断 owner-ready covered。 | `tools/knowledge-regression.sh` | Tool | `knowledge-hub-governance-regression-helper-20260619` |
