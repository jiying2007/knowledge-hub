...
| owner-handoff-packet-json | 当前 owner gate helper 输出一包化只读 handoff packet | `--owner project-owner --handoff-packet --json` 一次性输出 owner inbox、evidence readiness、forms JSONL 骨架、checklist 和命令序列；不生成 owner decision、不写 `.local.jsonl`、不关闭 gate |
...
| status-product-profile-blocks-noncanonical-residue | product profile 对非规范过程残留执行硬门禁 | 临时副本注入带封存流程标记的 reviewing item 后，`knowledge-status --final-profile product` 必须返回 `needs-fix`，并暴露 `product-noncanonical-items` blocker |
| final-gate-product-review-queue-owner-review-blocker | product final gate 将普通 AI/外部资料待复核队列归入 owner/evidence backlog | `knowledge-final-gate --final-profile product` 必须保持技术门禁 `pass`、整体 `needs-owner-review`，且 gap map 不得把该缺口标为 Codex 可自动完成 |
...
| final-gate-default-regression-path | product gate 的 full 路径消费模块化 140-result regression 契约 | 隔离夹具要求 gate 以 `--suite full --as-of <date>` 调用 regression，并返回 `checks.knowledge_regression.status=pass`、`result_count=140` 和 `full_regression_ready=true`；顶层 full regression 与最终真实 product gate 负责执行完整套件，避免测试内递归重复执行 |
...
| final-gap-readability-positive-contracts | 当前仓库不改 fixture，直接验证终态 gap/readability 正向路径 | `knowledge-check` 必须 pass，final gate 必须只停在 owner review；2026-06-21 后 governance audit、source policy `notes_zh` 和 source final-state 字段均完整 |
| final-gate-maintenance-entry-wording-no-section-drift | 当前 final gate 维护入口摘要不得依赖易漂移的章节编号 | `tools/knowledge-final-gate.sh` 必须使用“docs/goals 中列出的 8 类长期维护入口和 1 个离线维护包”描述运行时摘要，不得继续输出“第七节”章节口径 |
| owner-landing-plan-project-index | 当前 landing plan 和 landing audit 输出 owner 决策人工落地文件清单、worksheet 验证目录、验证命令和落地审计 | `required_manual_files` 包含 `pcr02-owner-decision-worksheets-20260618.jsonl`、`indexes/by-project.md` 和 `indexes/by-status.md`；landing audit 标记只读，要求人工更新 worksheet 行，并列出 registry/source-policy/index 手工 delta |
...
| owner-archive-only-explicit-path-contract | archive-only owner 表单接受明确 canonical archive 路径 | worksheet 007 选择 `owner_decision=archive-only` 且 `target_decision` 为 `projects/xcrz-sigmastar-demo/archive/...` 时必须通过表单一致性校验，不要求 owner 使用 worksheet 未列出的字面 `archive-only` 目标 |
| owner-archive-only-rejects-non-archive-target | 当前 archive-only owner 表单拒绝 validation / decisions 等非 archive 目标和旧入口 | worksheet 007 选择 `owner_decision=archive-only` 但 target 指向 `projects/pcr02/validation/` 或 `domains/projects/...` 时必须失败，并给出 `owner-decision-target-mismatch` 诊断 |
...
| owner-dispatch-source-scope-isolation | 临时副本把同一 owner 的两个 open worksheet 分到不同 `source_id` | `knowledge-owner-gates.sh --summary --json` 和 `knowledge-status.sh --json` 的 `owner_dispatch[]` 必须按 `source_id + owner` 分派，输出 `dispatch_scope_id/source_ids/mixed_source_owner`，且命令保留对应 `--source-id`，避免跨 source 合并后丢失 source scope |
| manifest-regression-count-capture-qualifier | 当前 2026-06-23 governance manifest 中的固定回归数量必须说明历史口径 | 含有固定回归数量的日期化 manifest 必须标注“当次运行历史捕获”或等价 live 输出口径，避免维护者把旧证据数量误读为当前 `knowledge-regression.sh --json` 的 live 数量 |
...
| manual-entry-default-dates | 当前人工新增向导输出默认日期 | registry / source policy 草稿填入 ISO 日期，不保留日期占位符 |
...
| by-topic-first-screen-readability-contract | 当前 `by-topic.md` 把恢复首屏、领域入口和历史治理台账分区 | 17 类恢复主题必须出现在历史台账前，历史 Knowledge Hub manifest 不得挤占恢复首屏，且索引中的 manifest 路径必须存在 |
...
| owner-target-materialization-read-only-no-source-dependency | 当前 PCR02 owner target 只读检查不依赖退役 origin 源文件 | `knowledge-pcr02-owner-targets.sh --json` 默认只读模式必须使用 owner decision landing manifest 的 source identity，返回 `target_present_count=4`、`blocked_count=0`，避免退役源文件缺失被误报为当前 Hub target 缺失 |
...
| knowledge-context-self-route | Hub 根目录自举查询解析为 Knowledge Hub 自身 route | `knowledge-context.sh --cwd ~/knowledge-hub --query "增强 Knowledge Hub 自举体验"` 必须输出 `repo_route.repo_id=knowledge-hub`、`route.project_id=knowledge-hub`、`domain_refs=root/governance` 和可用 canonical paths |
| knowledge-context-control-plane-query-override | Hub 控制面 cwd 下 query alias 优先路由到目标项目 | 在 `~/knowledge-hub` 内查询 PCR02 或 agent-dev-kit 时，`repo_route` 保持 Knowledge Hub，但 `route.project_id` 必须分别为 `pcr02` 和 `llm-agent` |
| knowledge-context-control-plane-alias-ambiguity | Hub 控制面混合 alias 歧义分流 | query 同时包含 `Knowledge Hub` 和 `PCR02` 时，`repo_route` 保持 Knowledge Hub，但 `route_selection.selected_project_id=pcr02` |
| embedded-asan-methodology-deprojectized | 团队级 ASAN 方法论保持去项目化和 active 范围边界 | `domains/embedded/runbooks/asan-debug-guide.md` 不得包含 PCR02-only 命令、程序名和部署路径，必须包含 ASAN_OPTIONS、BuildID、离线符号化、active 状态和不得提升到 embedded standards 的边界 |
...
| index-decision-registry-subsection-gate | 当前 `by-decision` 强门禁只锁 registry decision 覆盖 | 移除 registry decision 的索引引用会触发 `knowledge-check`，但不把 owner worksheet / source-policy decision 当作 registry decision 强约束 |
