...
当前 clean-slate 版本覆盖 135 个回归场景；已删除 source canonicalization 兼容路径、migration ledger 和 provenance record 模板相关测试。
