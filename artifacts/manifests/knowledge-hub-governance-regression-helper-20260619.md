...
| owner-archive-only-target-path-compatibility | 当前 archive-only owner 表单允许指向明确 canonical archive 路径 | worksheet 007 选择 `owner_decision=archive-only` 且 `target_decision` 为 `projects/pcr02/archive/...` 时必须通过表单一致性校验，不要求 owner 使用 worksheet 未列出的字面 `archive-only` 目标 |
