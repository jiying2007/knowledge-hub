...
| regression-manifest-coverage | 当前回归 helper manifest 覆盖所有回归 ID | manifest 包含当前全部回归场景和所有当前测试 ID |
