...
| status-next-owner-gate | 当前状态看板输出下一条 owner gate 聚焦命令 | JSON 中存在 `owner_gates.owner_dispatch[]`、`owner_gates.next_open.next_open_command` 和 `focus_command`；`next_actions_zh` 使用 `owner_gates.owner_dispatch[]`、`--next-open --checklist --forms`，兼容字段仍指向 `pcr02-owner-decision-worksheet-001` |
...
| owner-landing-plan-project-index | 当前 landing plan 输出 owner 决策人工落地文件清单、worksheet 验证目录和验证命令 | `required_manual_files` 包含 `indexes/by-project.md` 和 `indexes/by-status.md`，每个 step 带 `worksheet_verification_cwd` 与 `worksheet_verification_commands` |
...
| status-source-governance-summary | 当前状态看板 JSON 暴露 source coverage、review_after 和终态恢复命令 | `knowledge-status.sh --json` 必须输出 source 注册数、最新 source coverage manifest、latest coverage 选择依据、stale review_after 计数、review_after 复核命令、owner-ready package 覆盖率和 final gate 命令 |
