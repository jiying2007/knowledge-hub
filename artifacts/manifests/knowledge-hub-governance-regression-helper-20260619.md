...
| final-gate-owner-review-blocker | 当前终态 gate 聚合 check、regression、strict status 和结构化 gap map | `knowledge-check` 与 `knowledge-regression` 必须通过；当前只因 `owner-gates-open` 返回 `needs-owner-review`，并输出 `automatic_governance.status=complete-except-owner-review` 与 owner gap map |
