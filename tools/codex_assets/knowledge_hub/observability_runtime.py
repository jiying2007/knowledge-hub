...
def parse_traceparent(value: str) -> Dict[str, str]:
    ...
    return {"trace_id": trace_id, "parent_span_id": parent_span_id, "trace_flags": flags}


def make_traceparent(trace_id: str, span_id: str, *, sampled: bool = False) -> str:
...
def lane_health_projection(retrieval_payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    failures = []
    authorized = int(retrieval_payload.get("authorized_item_count", 0) or 0)
    if authorized > 0 and lexical == 0:
        failures.append("lexical-lane-empty")
    if authorized > 0 and dense == 0:
