...
fixture, mock, or synthetic data as production on its own: independently supplied
production provenance, runtime health, and cross-work traceability must all bind to
the same telemetry ledger before closure evidence can be emitted.
"""
...
def _validate_provenance(value: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if row.get("telemetry_scope") != "real-production-only":
        raise KnowledgeHubError("production telemetry_scope must be real-production-only")
    if _integer(row.get("synthetic_call_count"), "synthetic call count") != 0:
        raise KnowledgeHubError("production provenance must report zero synthetic calls")
    ...
    _sha256(row.get("call_ledger_sha256"), "provenance call ledger")
    _sha256(row.get("feedback_sha256"), "provenance feedback ledger")
    ...
    row["evidence_source_refs"] = [_text(item, "production evidence source ref") for item in refs]
...
def _validate_health(value: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    row["call_ledger_sha256"] = _sha256(row.get("call_ledger_sha256"), "health call ledger")
...
def _validate_trace(value: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    row["call_ledger_sha256"] = _sha256(row.get("call_ledger_sha256"), "trace call ledger")
...
def _adoption_observation(metrics: Mapping[str, Any], health: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    observation = {
        ...
        "valid_real_calls": _integer(usage.get("invocation_count"), "valid real calls"),
        "feedback_count": _integer(retrieval.get("feedback_count"), "explicit feedback count"),
        "p95_ms": max(
            _number(warm.get("search_p95_ms"), "search p95"),
            _number(warm.get("context_p95_ms"), "context p95"),
        ),
    }
    if health["p50_ms"] > observation["p95_ms"]:
...
def _assert_bindings(
    ...
    call_sha = _sha256(call_ledger_sha256, "call ledger")
    feedback_sha = _sha256(feedback_sha256, "feedback ledger")
    if provenance.get("call_ledger_sha256") != call_sha:
        raise KnowledgeHubError("production provenance is not bound to the call ledger")
    if provenance.get("feedback_sha256") != feedback_sha:
        raise KnowledgeHubError("production provenance is not bound to the feedback ledger")
    if health.get("call_ledger_sha256") != call_sha:
        raise KnowledgeHubError("production runtime health is not bound to the call ledger")
    if traceability.get("call_ledger_sha256") != call_sha:
        raise KnowledgeHubError("production traceability is not bound to the call ledger")
...
def _payload(
    ...
    return {
        ...
        "observed_at": provenance["window_end"],
        "evidence_source_refs": list(provenance["evidence_source_refs"]),
        "origin": {
            "classification": "real-production",
            "environment_id_sha256": provenance["environment_id_sha256"],
            "synthetic": False,
            "mock": False,
            "fixture": False,
        ...
        "adoption": {
            ...
            "valid_real_calls": observation["valid_real_calls"],
            "synthetic_calls": 0,
            "explicit_feedback_count": observation["feedback_count"],
            ...
            "traceability": {
                "status": "pass",
                "work_item_run_handoff_receipt_linked": traceability[
                    "work_item_run_handoff_receipt_linked"
                ],
            },
            "privacy": {
                "raw_query_stored": provenance["raw_query_stored"],
                "raw_task_stored": provenance["raw_task_stored"],
                "raw_prompt_stored": provenance["raw_prompt_stored"],
                "raw_content_stored": provenance["raw_content_stored"],
                "network_export_mode": provenance["network_export_mode"],
            },
        },
    }
...
def build_real_adoption_evidence(
    *,
    ...
    _assert_bindings(provenance_row, health_row, trace_row, call_ledger_sha256, feedback_sha256)
    observation = _adoption_observation(metrics, health_row)
    payload = _payload(
        provenance_row,
        health_row,
        trace_row,
        observation,
        source_revision,
        _sha256(call_ledger_sha256, "call ledger"),
        _sha256(feedback_sha256, "feedback ledger"),
    )
