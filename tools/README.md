...
- `knowledge-status.sh`: 只读控制面 dashboard。
  ...
  - 主要输出：`owner_dispatch[].owner_inbox_json_command` 是按 owner 恢复的默认单屏入口；`owner_dispatch[].handoff_packet_json_command` 是可交给 owner 的 one-shot JSON 包入口；`owner_dispatch[].suggested_owner_packet` 先排列 owner-inbox，再排列 summary、evidence-readiness、forms-jsonl、validate、landing-plan 和 landing-audit，且每一步都有 `notes_zh`；`owner_gates.source_identity_read_policy` 说明 dashboard 会通过 `knowledge-owner-gates.sh` 为 hash 只读读取 source 文件字节，但不复制正文、不写源项目、不生成 owner decision、不关闭 gate；`next_open_queue[]` 按 `review_after, worksheet_id` 给出 open gate 领取队列，其中 `owner_ready_package_status` 只来自 `knowledge-owner-gates.sh` 的逐行强校验字段，不用 registry item presence 推断 covered；`review_queues` 从 `registry/items.jsonl` 和 `registry/sources.json` 派生 AI 生成 / 外部资料待人工复核队列，`--review-queue-limit <N>` 只限制 status 输出 sample，不改变队列总数；`sources.latest_coverage_selection` 记录 latest source coverage closeout 选择策略；`sources.source_recovery_rows[]` 合成 source 当前状态恢复行。
