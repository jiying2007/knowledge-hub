...
- `knowledge-check.sh`: read-only validation, including registry field/path/enum checks, source enum checks, secret-pattern scan, core index drift checks for missing/stale item refs, and local path/glob reference checks in `indexes/*.md`.
