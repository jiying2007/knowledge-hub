...
| 终态检查 | 证明自动治理是否闭环，读取 `final_status` 并区分 `ok` / `needs-owner-review` / `needs-fix` | `knowledge-final-gate.sh --json`；正文最大收口使用 `knowledge-final-gate.sh --json --final-profile max-body` | 必须包含 regression、diff check、strict status；`standard` 不让普通人工复核队列阻断终态，`max-body` 会把待复核队列和不安全 source inventory 作为 blocker |
| 路径漂移 | 扫描旧归档路径在 Hub、Codex、memory、历史 session 和 skill/agent 资产中的残留，并按 policy/provenance/runtime/memory-superseded/historical 分类 | `knowledge-path-audit.sh --scope hub --json`、`knowledge-path-audit.sh --scope skills --json`、`knowledge-path-audit.sh --scope all --json` | 只读 report-only；不改写 memory、不改历史 session、不修改 `~/codex` 或源项目 |
...
- `knowledge-path-audit.sh`: 全局路径漂移只读审计入口。它扫描 `~/embedded/engineering_archive`、`~/codex/docs/archive` 等 retired 路径在 Hub、Codex、memory、历史 session 和 skill/agent 资产中的命中，并分类为 `canonical-policy`、`provenance`、`runtime-route-candidate`、`memory-superseded` 或 `historical-session`。默认 `--scope hub`，skill/agent 专项用 `--scope skills --json`，需要跨仓报告时用 `--scope all --max-matches <N> --json`；它只报告，不改写 memory、历史 session、`~/codex` 或源项目。
...
| 终态检查 | 通常不新增文件；需要保存证据时落相邻 manifest | `rtk git diff --check`、`knowledge-final-gate.sh --json`、正文最大收口用 `knowledge-final-gate.sh --json --final-profile max-body` |
