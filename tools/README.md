...
- `knowledge-final-gate.sh`: 仓库只读终态门禁。
  ...
  - 主要输出：JSON 包含 `today`、`as_of_source`、`automatic_governance`、`owner_recovery`、`final_state_audit`、`maintenance_entry_audit`、`linking_audit`、稳定字段 `proof_artifacts`、`source_check_runtime`、`highest_priority_rules_audit`、`evidence_index` 和 `gap_map`；旧字段 `proof_artifacts_20260622` 仅为兼容保留，新消费方优先读取 `proof_artifacts`。`owner_recovery.next_open_queue[]` 会透传 status dashboard 的下一批 open worksheet；`maintenance_entry_audit` 只证明 docs/goals 中列出的 8 类长期维护入口存在且边界清楚；`linking_audit` 只证明 registry/index/search 恢复链路覆盖跨会话、项目、source、topic 和 decision；`proof_artifacts` 证明终态 proof 主制品在 registry、migration 和核心索引中可恢复；`source_check_runtime` 当前执行一次 PCR02 Level 2 report-only 路径存在性检查；`highest_priority_rules_audit` 用 10 条高优先级规则说明哪些规则已由运行证据覆盖、哪些只能过程审计；`evidence_index` 逐条记录命令证据；纯 owner-review 终态还会包含 `owner-blocker-provenance`。
