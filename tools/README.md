...
- `knowledge-status.sh`: read-only control-plane dashboard; summarizes `knowledge-check`, registry counts, source coverage, migrations, stale review dates, owner gate status and the next open owner gate focus command. Use `--strict` as a final-state gate that returns non-zero unless the status is `ok`.
