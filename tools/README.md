...
| 人工复核 | 查看 AI 生成内容和外部资料待复核队列 | `knowledge-status.sh --json --review-queue-limit 10`、`knowledge-index-plan.sh --section review-queue --json` | 只从 registry 派生 read-only / report-only 队列，不自动填 human review 字段、不提升 active |
...
- `knowledge-status.sh`: 只读控制面 dashboard。
  ...
  - 主要输出：`owner_dispatch[].owner_inbox_json_command` 是按 owner 恢复的默认单屏入口；`owner_dispatch[].handoff_packet_json_command` 是可交给 owner 的 one-shot JSON 包入口；`owner_dispatch[].suggested_owner_packet` 先排列 owner-inbox，再排列 summary、evidence-readiness、forms-jsonl、validate、landing-plan 和 landing-audit；`next_open_queue[]` 按 `review_after, worksheet_id` 给出 open gate 领取队列，其中 `owner_ready_package_status` 只来自 `knowledge-owner-gates.sh` 的逐行强校验字段，不用 registry item presence 推断 covered；`review_queues` 从 `registry/items.jsonl` 和 `registry/sources.json` 派生 AI 生成 / 外部资料待人工复核队列，`--review-queue-limit <N>` 只限制 status 输出 sample，不改变队列总数；`sources.latest_coverage_selection` 记录 latest source coverage closeout 选择策略；`sources.source_recovery_rows[]` 合成 source 当前状态恢复行。
...
- `knowledge-final-gate.sh`: 仓库只读终态门禁。
  ...
  - 主要输出：JSON 包含 `today`、`as_of_source`、`automatic_governance`、`owner_recovery`、`review_queue_recovery`、`final_state_audit`、`maintenance_entry_audit`、`linking_audit`、稳定字段 `proof_artifacts`、`source_check_runtime`、`highest_priority_rules_audit`、`evidence_index` 和 `gap_map`；旧字段 `proof_artifacts_20260622` 仅为兼容保留，新消费方优先读取 `proof_artifacts`。`owner_recovery.next_open_queue[]` 会透传 status dashboard 的下一批 open worksheet；`review_queue_recovery` 只摘取 AI/外部资料人工复核队列的只读摘要和 index-plan 入口，普通待复核项不阻断 final gate；`maintenance_entry_audit` 只证明 docs/goals 中列出的 8 类长期维护入口存在且边界清楚；`linking_audit` 只证明 registry/index/search 恢复链路覆盖跨会话、项目、source、topic 和 decision；`proof_artifacts` 证明终态 proof 主制品在 registry、migration 和核心索引中可恢复；`source_check_runtime` 当前执行一次 PCR02 Level 2 report-only 路径存在性检查；`highest_priority_rules_audit` 用 10 条高优先级规则说明哪些规则已由运行证据覆盖、哪些只能过程审计；`evidence_index` 逐条记录命令证据；纯 owner-review 终态还会包含 `owner-blocker-provenance` 和 `review-queue-recovery`。
