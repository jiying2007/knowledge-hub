...
- `knowledge-promote.sh`: 默认 dry-run；`--apply` 必须同时提供有效 `--authorization-id`、真人填写的 `--forms`、当前正文 `--expected-sha256` 和预期原状态。工具只机械执行已授权决定，不生成 owner decision。
- `knowledge-retire.sh`: 默认 dry-run；受控 apply 使用同一 authorization/forms/hash/expected-state 契约，支持 archived/superseded/rejected，事务失败会定向回滚，不删除正文。
