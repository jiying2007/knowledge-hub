...
| 路径漂移 | 扫描旧归档路径在 Hub、Codex、memory 和历史 session 中的残留，并按 policy/provenance/runtime/historical 分类 | `knowledge-path-audit.sh --scope hub --json`、`knowledge-path-audit.sh --scope all --json` | 只读 report-only；不改写 memory、不改历史 session、不修改 `~/codex` 或源项目 |
