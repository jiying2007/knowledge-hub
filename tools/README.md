...
| 人工复核 | 查看 AI 生成内容和外部资料待复核队列 | `knowledge-status.sh --json --review-queue-limit 10`、`knowledge-index-plan.sh --section review-queue --json`、`knowledge-index-plan.sh --section review-queue --queue-forms-jsonl` | 只从 registry 派生 read-only / report-only 队列和人工填写前 JSONL 骨架，不自动填 human review 字段、不提升 active |
