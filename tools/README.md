...
- `knowledge-final-gate.sh`: repository read-only final-state gate; runs `knowledge-check --diagnostics`, `knowledge-regression --json` and `knowledge-status --strict --json` together so terminal validation cannot miss regression drift. JSON output includes `automatic_governance` and `gap_map` so automation and humans can distinguish `complete-except-owner-review` from real tool/registry/index failures. It returns non-zero unless all three gates are terminal-ok. The regression subcommand may create and clean temporary fixtures under `/tmp`; it must not modify Knowledge Hub content, registry, indexes or source project docs.
...
查看 JSON 中的 `automatic_governance.status` 和 `gap_map`：`complete-except-owner-review` 表示自动治理已闭环但仍需人工 owner decision；`needs-fix` 表示还有非 owner blocker 需要先修复。
