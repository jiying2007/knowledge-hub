...
- `knowledge-context.sh`: 跨项目会话的 Hub 上下文预检入口。它根据 `--cwd`、`--query` 和 `--task-type` 解析项目路由、Hub 入口、候选条目、search 结果和是否需要生成 candidate；`--context-budget small|normal|deep` 控制输出预算；JSON 中 `context.current/recent/related/risks` 是可解释上下文包，`ranked_items[].why_selected` 说明排序原因；不读取 raw session 全文、不写文件。
