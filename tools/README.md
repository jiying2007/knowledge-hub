...
- `knowledge-status.sh`: read-only control-plane dashboard; summarizes `knowledge-check`, registry counts, source coverage, migrations, stale review dates, owner gate status, all-open owner summary commands, owner forms JSONL commands, owner filled-form validation commands, no-write landing-plan commands, the next open owner gate commands and structured `strict_blockers`. Use `--strict` as a final-state gate that returns non-zero unless the status is `ok`.
