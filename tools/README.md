...
| 健康首屏 | 总数、summary 缺口、review queue、stale review、changed-only orphan、reviewing triage、mature blocker 和当前 final gate 摘要 | `knowledge-health-summary.sh --json --as-of 2026-07-11` | 只读；默认会运行 mature final gate quick regression 以给出当前 gate 状态；`--skip-final-gate` 只看 status dashboard |
...
| 终态检查 | 证明自动治理是否闭环，读取 `final_status` 并区分 `ok` / `needs-owner-review` / `needs-fix` | `knowledge-final-gate.sh --json` 默认 quick regression；正文最大收口使用 `knowledge-final-gate.sh --json --final-profile max-body --full-regression`；成熟态使用 `knowledge-final-gate.sh --json --final-profile mature --full-regression` | 必须包含 regression、diff check、strict status；`standard` 不让普通人工复核队列阻断终态，`max-body` / `mature` 会把普通待复核队列作为 owner-review 阻断，把 active/promotion 缺复核、不安全 source inventory 和迁移态残留作为 needs-fix blocker |
...
- `knowledge-check.sh`: 只读一致性门禁。
  - 用途：检查 registry JSON/JSONL、owner/project/topic/source 登记、核心索引、source policy、template 必填字段、secret-pattern、owner-gated active 阻断、AI provenance 和中文 diagnostics。
...
- `knowledge-health-summary.sh`: 只读健康首屏入口。它汇总 registry 总数、各状态 `summary_zh` 缺口、review queue pending、stale review_after、changed-only orphan 文件、reviewing triage、owner gate open、mature audit blocker 和当前 final gate 摘要；默认运行 `knowledge-final-gate.sh --json --final-profile mature` 的 quick regression，`--skip-final-gate` 可用于只看 status dashboard。它不写 registry/index/manifest，不生成 owner decision、不提升 active、不关闭 owner gate、不写 memory、不修改源项目。
- `knowledge-orphan-files.sh`: 只读正文孤岛检查入口。默认 `changed-only`，只检查当前 Git 改动里的 `projects/`、`domains/`、`governance/`、`notes/` Markdown 正文是否有 registry path；`--all` 暴露历史长尾 orphan，只作 advisory，不自动作为 mature blocker。`--strict` 只建议用于提交前检查本次新增正文。
- `knowledge-reviewing-triage.sh`: 只读 reviewing 周期 triage 入口。按 bucket、recommended_action、review_after 输出 reviewing 队列，帮助每周判断 keep-reviewing、evidence-needed、owner-review-and-validation 或 archive-ready-check；不自动 archive、不提升 active、不代签 owner。
...
- `knowledge-context.sh`: 跨项目会话的 Hub 上下文预检入口。它根据 `--cwd`、`--query` 和 `--task-type` 解析项目路由、Hub 入口、候选条目、search 结果和是否需要生成 candidate；`--context-budget small|normal|deep` 控制输出预算；JSON 中 `repo_route` 表示当前 cwd 所属仓库，`route` 表示 query 目标，Hub 控制面使用 `control-plane-query-aware` 策略避免在 `~/knowledge-hub` 内查询 PCR02/agent-dev-kit 时被自身路由吞掉；`canonical_paths`、`context.current/recent/related/risks` 是可解释上下文包，`ranked_items[].why_selected` 说明排序原因；全文检索无命中时会按关键词做 route-scoped fallback，并把结果放入 `context.search_fallback`；不读取 raw session 全文、不写文件。
...
- `knowledge-status.sh`: 只读控制面 dashboard。
  - 用途：汇总 `knowledge-check`、registry/source/source policy、owner gate、`owner_gates.owner_dispatch[]`、下一条 open gate、`owner_gates.next_open_queue[]`、`final_gate_command` 和 `strict_blockers`。
  - 主要输出：`owner_dispatch[].owner_inbox_json_command` 是按 owner 恢复的默认单屏入口；`owner_dispatch[].handoff_packet_json_command` 是可交给 owner 的 one-shot JSON 包入口；`owner_dispatch[].suggested_owner_packet` 先排列 owner-inbox，再排列 summary、evidence-readiness、forms-jsonl、validate、landing-plan 和 landing-audit，且每一步都有 `notes_zh`；`owner_gates.source_identity_read_policy` 说明 dashboard 会通过 `knowledge-owner-gates.sh` 为 hash 只读读取 source 文件字节，但不复制正文、不写源项目、不生成 owner decision、不关闭 gate；`next_open_queue[]` 按 `review_after, worksheet_id` 给出 open gate 领取队列，其中 `owner_ready_package_status` 只来自 `knowledge-owner-gates` 的逐行强校验字段，不用 registry item presence 推断 covered；`review_queues` 从 `registry/items.jsonl`、current source 和 retired source ledger 派生 AI 生成 / 外部资料待人工复核队列，`--review-queue-limit <N>` 只限制 status 输出 sample，不改变队列总数；`--final-profile max-body` / `--final-profile mature` 会把普通待复核队列作为 owner-review 阻断，把 `active` / promotion 缺人工复核、`needs-edits` / `defer` 复核结果和不安全 `copy-body` inventory 作为 needs-fix blocker；`--final-profile mature` 还会输出 `sources.mature_audit`，阻断 migration/copy-first/migrated/source-docs 等迁移态残留、copy-first 工具入口、已关闭迁移 source 主列表残留和高比例长期 `reviewing`；已归档且带 archive-only/tombstone/no-active-promotion 等边界的迁移证据只计入 sealed provenance，不视为当前残留；`sources.latest_coverage_selection` 记录 latest source coverage closeout 选择策略；`sources.source_recovery_rows[]` 合成 current + retired source 恢复行；`sources.max_body_source_inventory_audit` 说明正文最大迁移 profile 下每个 source inventory 是否还有 pending 或不安全 copy-body。
...
- `knowledge-regression.sh`: 只读回归 fixture 入口；把仓库复制到 `/tmp`，只修改临时副本，用于验证 status bucket mismatch、partial owner resolution 等关键负向门禁。默认 `--suite full` 保持完整覆盖；`--suite quick` 只跑日常门禁 smoke 集。默认每个场景后清理临时 fixture，内部异常会记录结构化失败细节；`--json` 模式输出 JSON，支持 `--as-of YYYY-MM-DD` / `KNOWLEDGE_TODAY` 固定日期敏感命令，并使用 `KNOWLEDGE_REGRESSION_MIN_TMP_FREE_BYTES` 做低空间预检。JSON 中的 `slowest_results[]` 和每条 result 的 `duration_sec` 用于定位 full suite 性能瓶颈，不参与 pass/fail 判定。
...
| 终态检查 | 通常不新增文件；需要保存证据时落相邻 manifest | `rtk git diff --check`、`knowledge-final-gate.sh --json`、正文最大收口用 `knowledge-final-gate.sh --json --final-profile max-body --full-regression` |
