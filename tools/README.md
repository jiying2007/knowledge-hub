...
| 上下文预检 | 从 cwd 和任务文本解析项目、Hub 入口、候选知识和写入建议 | `knowledge-context.sh --cwd "$PWD" --query "如何分析 core" --task-type debug --json` | 只读；用于其他项目会话先查 Hub，不读取 raw session 全文 |
...
- `knowledge-context.sh`: 跨项目会话的 Hub 上下文预检入口。它根据 `--cwd`、`--query` 和 `--task-type` 解析项目路由、Hub 入口、候选条目、search 结果和是否需要生成 candidate；不读取 raw session 全文、不写文件。
