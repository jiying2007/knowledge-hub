...
- `knowledge-status.sh`: read-only control-plane dashboard; summarizes `knowledge-check`, registry counts, source coverage, migrations, stale review dates, owner gate status, all-open owner summary commands and the next open owner gate commands. Use `--strict` as a final-state gate that returns non-zero unless the status is `ok`.
