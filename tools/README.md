...
| 人工复核 | 查看 AI 生成内容和外部资料待复核队列 | `knowledge-status.sh --json --review-queue-limit 10`、`knowledge-index-plan.sh --section review-queue --json`、`knowledge-index-plan.sh --section review-queue --queue-forms-jsonl`、`knowledge-index-plan.sh --section review-queue --validate-queue-forms <jsonl> --json` | 只从 registry 派生 read-only / report-only 队列、人工填写前 JSONL 骨架和结构校验，不写 registry、不自动填 human review 字段、不生成 review 结论、不提升 active、不关闭 owner gate |
...
- `knowledge-final-gate.sh`: 仓库只读终态门禁。
  ...
  - 主要输出：JSON 包含 `today`、`as_of_source`、顶层 `summary`、`automatic_governance`、`owner_recovery`、`review_queue_recovery`、`final_state_audit`、`maintenance_entry_audit`、`linking_audit`、稳定字段 `proof_artifacts`、`source_check_runtime`、`highest_priority_rules_audit`、`evidence_index` 和 `gap_map`；旧字段 `proof_artifacts_20260622` 仅为兼容保留，新消费方优先读取 `proof_artifacts`。`summary` 只从已有运行字段派生，用于扫读 Level 1/2/3、长期维护入口、proof 和 linking 状态，不参与 `final_status` 判定；`owner_recovery.next_open_queue[]` 会透传 status dashboard 的下一批 open worksheet；`review_queue_recovery` 只摘取 AI/外部资料人工复核队列的只读摘要、index-plan 入口、推荐批次领取命令、`--queue-forms-jsonl` 表单骨架命令和 `--validate-queue-forms` report-only 校验模板，普通待复核项不阻断 final gate；`maintenance_entry_audit` 证明 docs/goals 中列出的 8 类长期维护入口和 1 个离线维护包存在且边界清楚，并带 `requirement_refs` / `section_refs`；`linking_audit` 只证明 registry/index/search 恢复链路覆盖跨会话、项目、source、topic 和 decision，并提供 `summary`、`requirement_refs` / `section_refs`；`proof_artifacts` 证明终态 proof 主制品在 registry、migration 和核心索引中可恢复，并标注覆盖章节；`source_check_runtime` 当前执行一次 PCR02 Level 2 report-only 路径存在性检查；`highest_priority_rules_audit` 用 10 条高优先级规则说明哪些规则已由运行证据覆盖、哪些只能过程审计；`evidence_index` 逐条记录命令证据、`requirement_refs` 和 `section_refs`；纯 owner-review 终态还会包含 `owner-blocker-provenance` 和 `review-queue-recovery`。
