...
try:
    ...
    if target_prefix.parts[:3] != ("domains", "projects", "pcr02"):
        errors.append(f"target-prefix outside domains/projects/pcr02: {args.target_prefix}")
    elif len(target_prefix.parts) < 4 or target_prefix.parts[3] != args.bucket:
        errors.append(f"target-prefix bucket mismatch: bucket={args.bucket} path={args.target_prefix}")
except Exception as exc:
