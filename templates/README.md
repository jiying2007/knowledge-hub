...
|---|---|---|---|---|---|
| 通用新增条目 | `id`、`title`、`kind`、`domain`、`path`、`scope`、`visibility`、`status`、`owner`、`source`、`review_after`、`created_at`、`updated_at`、`promotion`、`tags`、`validation_refs`、`summary_zh`、语言/术语/evidence 字段 | `artifact_refs`、`related`、`promotion_decision`、`manual_validation_pending` 正文块 | 未验证前不得填 `status=active`，不得把 `promotion` 改成非 `none` | 不得伪造 source、owner、验证命令、hash、review 结果 | `rtk bash tools/knowledge-new.sh ...`；`rtk bash tools/knowledge-check.sh --dry-run --json --diagnostics` |
