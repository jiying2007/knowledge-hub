# pcr02-project-scratch Source 策略

## 终态

该 source 必须通过 `sources/pcr02-project-scratch/` 在 Hub 内可恢复、可搜索、可审计。

## 当前策略

- `registry/sources.json` 的 `path` 已收敛到 `sources/pcr02-project-scratch`。
- 当前知识入口只使用 Hub 内路径。
