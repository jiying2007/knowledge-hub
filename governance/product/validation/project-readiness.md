...
| LLM Agent 精确源码门禁 | 根仓 + 7 个 gitlink 精确隔离 worktree，执行 smoke/quick/full、主工作区 live-corpus 与远端恢复校验 | 修复前 Full 61/62 的唯一阻塞已闭环；`150fdee` 已 fast-forward 发布到 `origin/main`，推送前隔离与推送后当前主机远端 clone Full 均为 62/62，ADK 52/52、主工作区 313 篇 live-corpus 通过，七个 gitlink 未变。私有 `agent-dev-kit` 在非交互 HTTPS 下失败，临时 SSH override 后恢复；正式 artifact、release、rollback 仍未闭环。 |
...
| B5 软件发布波次 | `agent-dev-kit`、`firmware-release-tools`、`llm-agent` | 各缺 artifact/release/rollback；`firmware-toolchains` 缺 release/rollback；`llm-agent@150fdee` 的源码远端发布与当前主机远端恢复已闭环，但私有 submodule 零配置跨主机恢复仍未证明 | 发布 owner 绑定目标版本、正式制品 URI/SHA、发布记录与隔离恢复；`llm-agent` 还需固化私有 submodule bootstrap 并在另一台主机复验。 | `knowledge-final-gate.sh --final-profile product --json` |
...
2026-07-17 续跑核验：刷新 `local/workspaces.json` 后，29/29 注册远端仍可定位；`llm_agent` 本地与 `origin/main` 均为 `150fdee1509b899bbb0d8c0762b7a9313b63ba94`，分叉 `0 0`，当前主机远端 clone Full 62/62。B1 前三项整文件 SHA256 未变化；第四项因追加 source publish、远端恢复及 HTTPS 负证据，当前整文件 SHA256 更新为 `4660c78a574d0d1c011f10577f3d69a318838cc172dc2144966b0ae6726f6d20`，此前 hash 均已陈旧，不得用于人工复核绑定。Codex 只更新了可验证事实和风险边界，没有替真人填写复核字段或改变 reviewing/pending 状态。
...
rtk bash ~/knowledge-hub/tools/knowledge-project-readiness.sh --check --as-of 2026-07-17 --json
rtk bash ~/knowledge-hub/tools/knowledge-final-gate.sh --json --final-profile product --regression-suite full --as-of 2026-07-17 --require-terminal
```

## Goal Closure

- 自动化目标：发布锚点 `9a1a363` 已证明控制面可交付；本轮继续修复可复现的契约、性能和验证问题，包括普通 review queue 的整文件 hash 绑定，并分批落地可由本机或隔离环境产生的真实 release/rollback evidence。门禁全绿后允许创建本地 commit，但不自动 push/tag/release。
...
- 当前决策：历史普通复核、30 项 authority-boundary、3 项 PCR02 专项 owner 绑定和发布锚点 `9a1a363` 均已完成；当前保留 4 条 schema v2、整文件 hash-bound 精确内容复核队列。`llm-agent` 本地 Full 技术阻塞已关闭，但没有把它扩大解释为远端发布或 evidence-ready。长期终态继续保持 `evidence-pending`，直到真实责任人和实际环境补齐项目证据、生命周期决定与采用门槛；外部缺口已收敛为上方 blocker packet。
